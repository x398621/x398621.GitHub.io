(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "12px 微软雅黑,Verdana";         //修改点击按钮的字体和文字
            ctx.fillText(lover.info.name, 21, 16);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    function handleErr(msg,url,l)   //页面出错自动处理模块
    {
        if(lover.info.debug == true)    //开启了调试模式
        {
            txt="页面加载出错！\n\n";
            txt+="错误内容: \n" + msg + "\n\n";
            txt+="错误链接: \n" + url + "\n\n";
        ...
        else
        {
            ...
            divObj.innerHTML = "<div style=\"text-align: center;\">检测到页面加载失败，自动刷新中……</div>"; 
